...
root= r'/home/zean/wxbot_project_py2.7'
...
class WeChatMsgProcessor(object):
    ...
    def __init__(self, db):
        print '\n[*] 初始化 WeChatMsgProcessor 处理微信消息的Handler'
        self.db = db
    ...
    def handle_command(self, cmd, msg):
        ...
        wechat = self.wechat
        g_id = ''
        print '\n[*] 处理发给自己的消息cmd=',cmd,',msg=',msg
        for g in wechat.GroupList:
            print '遍历所有的名字', g['NickName']
            if g['NickName'] == msg['group_name']:
                print '找到对象, ', g['UserName']
                g_id = g['UserName']
        ...
        if cmd == 'test':
            ...
            emot_path = os.path.join('/','home','zean','wxbot_project_py2.7', 'test', 'emotions', name)
            wechat.send_emot(g_id, emot_path)

    def check_schedule_task(self):
        # update group member list at 00:00 am every morning
        print '[*] check_schedule_task 执行定时任务,同步group member list'
        t = time.localtime()
