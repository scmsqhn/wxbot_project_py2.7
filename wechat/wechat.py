...
from wxconfig import ConfigManager
from wxconfig import Constant
from wxconfig import Log
#---------------------------------------------------
import json
import re
import sys
...
class WeChat(WXAPI):
    ...
    def __init__(self, host='wx.qq.com'):
        ...
        self.db = None
        self.save_data_folder = ''  # 保存图片，语音，小视频的文件
    ...
    def handle_mod(self, r):
        ...
        # ModContactList: 变更联系人或群聊列表，或群名称
    ...
    def readMsg(self, content, msg):#content=str, msg=dict
      print '\n[*] 读取信息,对信息进行分类'
      print '\n[*] content=', content
      print '\n[*] msg[username]=', msg
      pass
    ...
    def handle_msg(self, r):
        print '\n[*] 处理信息wechat.wechat.handle_msg'
        print '\n[*] r= ', r
        """
        @brief      Recover from snapshot data.
        @param      r  Dict: message json
        """
        print '\n[*] 判断是否为空, self.msg_handler= ', self.msg_handler
        if self.msg_handler:
        ...
        for msg in r['AddMsgList']:
            print "\n[*] 遍历 r['AddMsgList'] msg= ", msg
            msgType = msg['MsgType']
            msgId = msg['MsgId']
            content = msg['Content'].replace('&lt;', '<').replace('&gt;', '>')
            raw_msg = None
            print '\n[*] 打印msg信息,msgType=',msgType,';msgId=',msgId,';content=',content
            ...
            if msgType == self.wx_conf['MSGTYPE_TEXT']:
                ...
                # 普通文本消息
                    ...
                    ''''' 使用图灵接口,调用,获得信息
                    秦海宁170407
                    self.readMsg(content, msg['FromUserName'])
                    ans = self._tuling123(content)
                    print '\n[*] 访问tuling123 返回信息,', ans
                    if self.webwxsendmsg(ans, msg['FromUserName']):
                        print '\n[*] 自动回复,',msg['FromUserName'], ans
                    else:
                        print '\n自动回复失败
                    '''
                    '''''使用自定义处理文本信息
                    ...
                    strcontent = content
                    print strcontent
                    print chardet.detect(strcontent)
                    ans = self.ananTextHandler(strcontent)
                    print '\n[*] 由ananTextHandler处理并返回信息', ans
                    if self.webwxsendmsg(ans, msg['FromUserName']):
                        print '\n[*] 自动回复,',msg['FromUserName'], ans
                    else:
                        print '\由ananTextHandler回复失败'
    ...
    def show_msg(self, message):
        ...
        if msg and msg['raw_msg']:
            ...
            if msg['raw_msg']['FromUserName'][:2] == '@@':
                # 接收到来自群的消息                g_id = msg['raw_msg']['FromUserName']
                group = self.get_group_by_id(g_id)
